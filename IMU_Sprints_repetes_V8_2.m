...
% exemple de chemain d'accès DATA_exemples\001_014_CL\Sprints_repetes\IMU

% Dans cet exemple, le dossier à appeler au début du programme est "DATA_exemples"
...
for g=1:size(listing_G)
    clearvars -except folder_G listing_G file_info Infos_roue result g
    ...
    for f = 1: size(listing,1)
        clearvars -except folder_G listing_G file_info Infos_roue result listing path_g nom_dos rayon_roue FrHZ camber_angle g f
...
%                    
                    ...
                    [zz, ~]=find(lgt==min(lgt));
                    zz=zz(1,1);
                    DATAR1= DATAR1(1:lgt(zz,1),:);
                    DATAR2= DATAR2(1:lgt(zz,1),:);
                    sizeDATA=size(DATAR1);
...
%                     clear END_Events PicvitR vmoyR1stab vmoyR2stab vmoystab vmaxR1start1 vmaxR2start1 Vmoymax1 vmaxR1start2 vmaxR2start2 Vmoymax2 vmaxR1start3 vmaxR2start3 Vmoymax3 vmaxR1stab vmaxR2stab vmaxmoystab Vmax Vmoy accstart Accmax Accmoy accstab loc_max Inttps tpsPR1 tpsPR1 tpsP_mean tpsReR1 tpsReR2 tpsRe_mean tpscycleR1 tpscycleR2 tpscycle_mean Asy cadenceR1 cadenceR2 cadence min_tps
                    for E=1:numel(Events)-1 
                        ...
                        % ajout pic max de fin si la courbe fini sur une
                        % monté
                        if peak_minR1(end)-peak_maxR1(end)>0
                            peak_maxR1(end+1)=length(VPPP);
                            add=1;
                        else
                            add=0;
                        end
                        ...
                        % ajout pic max de fin si la courbe fini sur une
                        % monté
                        if peak_minR2(end)-peak_maxR2(end)>0
                            peak_maxR2(end+1)=length(VPPP);
                            add=1;
                        else
                            add=0;
                        end
                        ...
                        % roue droite
                        BUTADMAXR1 = uicontrol('Units','Normalized','BackgroundColor','c','FontSize',12,'Position',[.92 .835 .08 .08],'String','Ajouter pic max R1','Style','pushbutton',...
                            'callback','MODIF=1;clear x1 y1;sgtitle(''Clic Gauche sur la courbe pour ajouter des pics max'');[x1,y1,bouton_souris] = ginput; for ii=1:length(x1);if round(x1(ii)+(0.1*FrHZ))<length(VPPP);[~,pmaxr1(end+1,1)]=max(VPPP(round(x1(ii)-(0.1*FrHZ)):round(x1(ii)+(0.1*FrHZ)),1));pmaxr1(end,1)=pmaxr1(end,1)+round(x1(ii)-(0.1*FrHZ))-1;else;[~,pmaxr1(end+1,1)]=max(VPPP(round(x1(ii)-(0.1*FrHZ)):end-1,1));pmaxr1(end,1)=pmaxr1(end,1)+round(x1(ii)-(0.1*FrHZ))-1;end;end;sp1;delete(ppmax1);hold on;pmaxr1=sort(pmaxr1) ;sp1=subplot(2,1,1); hold on;ppmax1=plot(pmaxr1(:,1),(VPPP(pmaxr1(:,1),1)),''o'',''MarkerFaceColor'',''c'',''MarkerSize'',10,''DisplayName'',''right pic max'');legend (''Position'',[.83 .60 .06 .06]);');
                        BUTADMINR1 = uicontrol('Units','Normalized','BackgroundColor','m','FontSize',12,'Position',[.92 .755 .08 .08],'String','Ajouter pic min R1','Style','pushbutton',...
                            'callback','MODIF=1;clear x1 y1;sgtitle(''Clic Gauche sur la courbe pour ajouter des pics min'');[x1,y1,bouton_souris] = ginput; for ii=1:length(x1);if round(x1(ii)+(0.1*FrHZ))<length(VPPP);[~,pminr1(end+1,1)]=min(VPPP(round(x1(ii)-(0.1*FrHZ)):round(x1(ii)+(0.1*FrHZ)),1));pminr1(end,1)=pminr1(end,1)+round(x1(ii)-(0.1*FrHZ))-1;else;[~,pminr1(end+1,1)]=min(VPPP(round(x1(ii)-(0.1*FrHZ)):end-1,1));pminr1(end,1)=pminr1(end,1)+round(x1(ii)-(0.1*FrHZ))-1;end;end;sp1;delete(ppmin1);hold on;pminr1=sort(pminr1) ;sp1=subplot(2,1,1); hold on;ppmin1=plot(pminr1(:,1),(VPPP(pminr1(:,1),1)),''o'',''MarkerFaceColor'',''m'',''MarkerSize'',10,''DisplayName'',''right pic min'');legend (''Position'',[.83 .60 .06 .06]);');
                        BUTSUPMAXR1 = uicontrol('Units','Normalized','BackgroundColor','c','FontSize',12,'Position',[.92 .675 .08 .08],'String','Supprimer pic max R1','Style','pushbutton',...
                            'callback','MODIF=1;clear x1 y1 indexpos;sgtitle(''Clic Gauche sur la courbe pour supprimer des pics max'');[x1,y1,bouton_souris] = ginput; for ii=1:length(x1);indexpos= find(abs(pmaxr1-x1(ii,1))==min(abs(pmaxr1-x1(ii,1)))); pmaxr1(indexpos)=[];end;sp1;delete(ppmax1);hold on;pmaxr1=sort(pmaxr1) ;sp1=subplot(2,1,1); hold on;ppmax1=plot(pmaxr1(:,1),(VPPP(pmaxr1(:,1),1)),''o'',''MarkerFaceColor'',''c'',''MarkerSize'',10,''DisplayName'',''right pic max'');legend (''Position'',[.83 .60 .06 .06]);');
                        BUTSUPMINR1 = uicontrol('Units','Normalized','BackgroundColor','m','FontSize',12,'Position',[.92 .595 .08 .08],'String','Supprimer pic min R1','Style','pushbutton',...
                            'callback','MODIF=1;clear x1 y1 indexpos;sgtitle(''Clic Gauche sur la courbe pour supprimer des pics min'');[x1,y1,bouton_souris] = ginput; for ii=1:length(x1);indexpos= find(abs(pminr1-x1(ii,1))==min(abs(pminr1-x1(ii,1)))); pminr1(indexpos)=[];end;sp1;delete(ppmin1);hold on;pminr1=sort(pminr1) ;sp1=subplot(2,1,1); hold on;ppmin1=plot(pminr1(:,1),(VPPP(pminr1(:,1),1)),''o'',''MarkerFaceColor'',''m'',''MarkerSize'',10,''DisplayName'',''right pic min'');legend (''Position'',[.83 .60 .06 .06]);');
                        % roue gauche
                        BUTADMAXR2 = uicontrol('Units','Normalized','BackgroundColor','b','FontSize',12,'Position',[.92 .37 .08 .08],'String','Ajouter pic max R2','Style','pushbutton',...
                            'callback','MODIF=1;clear x1 y1;sgtitle(''Clic Gauche sur la courbe pour ajouter des pics max'');[x1,y1,bouton_souris] = ginput; for ii=1:length(x1);if round(x1(ii)+(0.1*FrHZ))<length(VPPP);[~,pmaxr2(end+1,1)]=max(VPPP(round(x1(ii)-(0.1*FrHZ)):round(x1(ii)+(0.1*FrHZ)),2));pmaxr2(end,1)=pmaxr2(end,1)+round(x1(ii)-(0.1*FrHZ))-1;else;[~,pmaxr2(end+1,1)]=max(VPPP(round(x1(ii)-(0.1*FrHZ)):end-1,2));pmaxr2(end,1)=pmaxr2(end,1)+round(x1(ii)-(0.1*FrHZ))-1;end;end;sp2;delete(ppmax2);hold on;pmaxr2=sort(pmaxr2) ;sp2=subplot(2,1,2); hold on;ppmax2=plot(pmaxr2(:,1),(VPPP(pmaxr2(:,1),2)),''o'',''MarkerFaceColor'',''b'',''MarkerSize'',10,''DisplayName'',''right pic max'');legend (''Position'',[.83 .60 .06 .06]);');
                        BUTADMINR2 = uicontrol('Units','Normalized','BackgroundColor','r','FontSize',12,'Position',[.92 .29 .08 .08],'String','Ajouter pic min r2','Style','pushbutton',...
                            'callback','MODIF=1;clear x1 y1;sgtitle(''Clic Gauche sur la courbe pour ajouter des pics min'');[x1,y1,bouton_souris] = ginput; for ii=1:length(x1);if round(x1(ii)+(0.1*FrHZ))<length(VPPP);[~,pminr2(end+1,1)]=min(VPPP(round(x1(ii)-(0.1*FrHZ)):round(x1(ii)+(0.1*FrHZ)),2));pminr2(end,1)=pminr2(end,1)+round(x1(ii)-(0.1*FrHZ))-1;else;[~,pminr2(end+1,1)]=min(VPPP(round(x1(ii)-(0.1*FrHZ)):end-1,2));pminr2(end,1)=pminr2(end,1)+round(x1(ii)-(0.1*FrHZ))-1;end;end;sp2;delete(ppmin2);hold on;pminr2=sort(pminr2) ;sp2=subplot(2,1,2); hold on;ppmin2=plot(pminr2(:,1),(VPPP(pminr2(:,1),2)),''o'',''MarkerFaceColor'',''r'',''MarkerSize'',10,''DisplayName'',''right pic min'');legend (''Position'',[.83 .60 .06 .06]);');
                        BUTSUPMAXR2 = uicontrol('Units','Normalized','BackgroundColor','b','FontSize',12,'Position',[.92 .21 .08 .08],'String','Supprimer pic max R2','Style','pushbutton',...
                            'callback','MODIF=1;clear x1 y1 indexpos;sgtitle(''Clic Gauche sur la courbe pour supprimer des pics max'');[x1,y1,bouton_souris] = ginput; for ii=1:length(x1);indexpos= find(abs(pmaxr2-x1(ii,1))==min(abs(pmaxr2-x1(ii,1)))); pmaxr2(indexpos)=[];end;sp2;delete(ppmax2);hold on;pmaxr2=sort(pmaxr2) ;sp2=subplot(2,1,2); hold on;ppmax2=plot(pmaxr2(:,1),(VPPP(pmaxr2(:,1),2)),''o'',''MarkerFaceColor'',''b'',''MarkerSize'',10,''DisplayName'',''right pic max'');legend (''Position'',[.83 .18 .06 .06]);');
                        BUTSUPMINR2 = uicontrol('Units','Normalized','BackgroundColor','r','FontSize',12,'Position',[.92 .13 .08 .08],'String','Supprimer pic min R2','Style','pushbutton',...
                            'callback','MODIF=1;clear x1 y1 indexpos;sgtitle(''Clic Gauche sur la courbe pour supprimer des pics min'');[x1,y1,bouton_souris] = ginput; for ii=1:length(x1);indexpos= find(abs(pminr2-x1(ii,1))==min(abs(pminr2-x1(ii,1)))); pminr2(indexpos)=[];end;sp2;delete(ppmin2);hold on;pminr2=sort(pminr2) ;sp2=subplot(2,1,2); hold on;ppmin2=plot(pminr2(:,1),(VPPP(pminr2(:,1),2)),''o'',''MarkerFaceColor'',''r'',''MarkerSize'',10,''DisplayName'',''right pic min'');legend (''Position'',[.83 .18 .06 .06]);');
                        % validation
                        ...
                        cadence(E,2)=(mean(cell2mat(Acc_push(E+65,:)))+mean(cell2mat(Acc_push(E+73,:))))/2;
                        ...
                        elseif add==1
                            plot(peak_maxR1(1:end-1,1),(peak_maxR1(1:end-1,2)),'o','MarkerFaceColor','c','MarkerSize',10,'DisplayName','right pic max')
                            plot(peak_maxR2(1:end-1,1),(peak_maxR2(1:end-1,2)),'o','MarkerFaceColor','b','MarkerSize',10,'DisplayName','left pic max')    
                        end
...
%                         plot(mat_dist(1:END_Events(E,1)-Events(E,1)+1),time_fig,'Color',cell2mat(col{E,1}),'HandleVisibility','off')
                    ...
                    xlswrite(FileName_Rapport,nom_dos,'Rapport Athlète','C12');
